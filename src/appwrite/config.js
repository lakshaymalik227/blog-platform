...
export class Service{
    ...
    async getPosts(userId = null, queries = []) {
    try {
        queries.push(Query.equal('status', 'active')); // only active posts

        if (userId) {
            queries.push(Query.equal('userId', userId)); // only add user filter if provided
        }
